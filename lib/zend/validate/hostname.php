...
class mail_bank_Zend_Validate_Hostname extends mail_bank_Zend_Validate_Abstract {
   ...
   public function isValid($value) {
	 ...
	 do {
	    // First check TLD
	    $matches = array();
	    if (preg_match('/([^.]{2,63})$/iu', end($domainParts), $matches) || (array_key_exists(end($domainParts), $this->_validIdns))) {
	       reset($domainParts);
		  ...
		  if (!in_array(strtolower($this->_tld), $this->_validTlds) && !in_array($this->_tld, $this->_validTlds)) {
